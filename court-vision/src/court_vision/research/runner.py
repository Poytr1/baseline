...
def run_clip(
    ...
    keys["ingest"] = hashlib.sha1(f"{clip.video}:{frame_seq.total_frames}:{config.pipeline.target_resolution}".encode()).hexdigest()[:12]
    timings["ingest"] = time.time() - t0
    log(f"[ingest] {frame_seq.total_frames} frames @ {frame_seq.fps:.2f} fps ({timings['ingest']:.1f}s)")

    segments = cached(
        "scene",
        lambda: stage_scene(frame_seq, config),
    ...
    court = cached(
        "court",
        lambda: stage_court(frame_seq, segments, config),
    ...
    def compute_ball_raw():
        ...
        for i, seg in enumerate(segments):
            out.append(detect_ball_sequence(
                frame_seq.frames_dir, seg.start_frame, seg.end_frame,
    ...
    def compute_players():
        ...
        for i, seg in enumerate(segments):
            ...
            out.extend(detect_players_segment(
                ...
                far_crop=p.player_far_crop, max_court_x=p.player_max_court_x, max_court_y=p.player_max_court_y,
            ))
        return out
